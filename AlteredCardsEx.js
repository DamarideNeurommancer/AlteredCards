...
//alert("showRelated mode: "+mode);
 ...
 if(RelatedCards != ""){
  cell=row.insertCell(-1);
  cell.style.border="0";
  cell.innerHTML=`<button id='myBtnRelated' onclick='myRelated(${index})' title='Take a picture of this card with their related'>&#x1F4F7</button>`;
 }
 if(mode==0){
  rowMaster.scrollIntoView({behavior: "auto"});
  row.scrollTo(0,0);
...
function myRelated(index){
 var rowMaster=table.rows[index];
 var xmlindex=rowMaster.cells[3].innerText;
 var book=catalog.childNodes[xmlindex];
 var CardID=book.attributes[0].nodeValue;
 var RelatedCards=CardID+";"+book.attributes[3].nodeValue;
 //if(RelatedCards=="")
 // return;
