...
function getASImageURL(pageURL){
 fetch(pageURL).then(function(response){
	...
	let html=stringToHTML(text);
	 var picture = html.getElementsByClassName("product-slider-image product-slider-image__printing")[0].getElementsByTagName("img")[0];
	 alert("AS image URL is: "+picture.src);
  }).catch(function (err) {
	 // There was an error
	 console.warn(err);
 });
