...
async function tuneUp(text,wait=500,wave="square",draw=true){
 var spartito="Freq(Hz)\tNOTE\tOCTAVE\tChar\n";
 var text1=text.toUpperCase();
 const compressor=audio.createDynamicsCompressor();
 compressor.connect(audio.destination);
 const len=text1.length;
 for(var i=0;i<len;i++){
  var c=text1.charCodeAt(i);
  if(c<65||c>90){
   switch(c){
    case 32:
  ...
  var freqIdx=c-65;
  //Score
  const frequency=(arFreqEx[freqIdx][0].toFixed(3)).padEnd(8,' ');
  const note=(arFreqEx[freqIdx][1]).padEnd(4,' ');
  const oct=arFreqEx[freqIdx][2].toString().padEnd(6,' ');
  const letter=text.charAt(i);
  spartito+=frequency+"\t"+note+"\t"+oct+"\t"+letter+"\n";
  MusicalScore.innerHTML+=text.charAt(i)+'\u{1F3B5}';
  var when=0.95;
  if(letter==letter.toUpperCase()||letter==','||letter==' ')
   when=1.40;
  //if(i==len-1)
  // when=0.25;
